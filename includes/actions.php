<?php
	...
	function pw_rpt_fetch_data() {
		...
		// Parse input first, then sanitize individual values
		// DO NOT use sanitize_text_field on the entire string as it breaks array syntax []
		$postdata = isset( $_REQUEST['postdata'] ) ? wp_unslash( $_REQUEST['postdata'] ) : '';
		parse_str( $postdata, $my_array_of_vars );
		...
		$allowed_tables = array(
			'product', 'category', 'coupon', 'customer', 'details', 'orderstatus', 'paymentgateway',
			'billingcountry', 'billingstate', 'billingcity', 'recentorder', 'taxreport', 'profit',
			'refunddetails', 'stock_list', 'stock_min_level', 'stock_max_level', 'stock_zero_level',
			'abandoned_cart', 'abandoned_product', 'customer_analysis', 'coupon_discount',
			'customer_guest', 'customer_category', 'customer_min_max', 'customer_no_purchased',
			'customer_order_frequently', 'customerbuyproducts', 'custom_taxonomy', 'clinic',
			'country_per_month', 'ord_status_per_month', 'order_per_country', 'order_per_custom_shipping',
			'order_product_analysis', 'order_variation_analysis', 'order_status_change',
			'payment_per_month', 'prod_per_country', 'prod_per_state', 'prod_per_month',
			'product_per_users', 'product_variation_qty', 'projected_actual_sale', 'stock_list_sales',
			'stock_summary_avg', 'summary_per_month', 'variation_per_month', 'variation_stock',
			'tax_reports', 'details_full', 'details_combined', 'details_depot', 'details_full_shipping',
			'details_full_shipping_tax', 'details_order_country', 'details_tax_field', 'details_tickera',
			'details_user_id', 'brand', 'tags', 'variation',
			'int_reports_sales', 'int_reports_products', 'int_reports_customers', 'int_reports_home',
			'int_reports_transactions',
		);

		if ( ! in_array( $table_name, $allowed_tables, true ) ) {
			PW_Report_AWR_Helpers::json_error( 'Invalid table name', 400 );
		}

		global $pw_rpt_main_class;

		// Route to appropriate handler
		$intelligence_tables = array( 'int_reports_sales', 'int_reports_products', 'int_reports_customers', 'int_reports_home', 'int_reports_transactions' );
		if ( in_array( $table_name, $intelligence_tables, true ) ) {
	...
	/**
	 * AJAX Handler: Intelligence Customer Details
	 *
	 * @since 6.1
	 * @return void
	 */
	...
	function pw_rpt_int_customer_details() {
		...
		$order_id = isset( $my_array_of_vars['row_id'] ) ? absint( $my_array_of_vars['row_id'] ) : 0;

		if ( ! $order_id ) {
			PW_Report_AWR_Helpers::json_error( 'Invalid order ID', 400 );
		}

		global $pw_rpt_main_class;
		include( __PW_REPORT_WCREPORT_ROOT_DIR__ . '/includes/Intelligence/fetch_data_int_reports_orders_details.php' );
	...
	/**
	 * AJAX Handler: Add Note to Intelligence Report
	 *
	 * @since 6.1
	 * @return void
	 */
	...
	function pw_rpt_int_add_note() {
		...
		$id        = isset( $my_array_of_vars['id'] ) ? absint( $my_array_of_vars['id'] ) : 0;
		$target    = isset( $my_array_of_vars['target'] ) ? sanitize_text_field( $my_array_of_vars['target'] ) : '';
		$note_text = isset( $my_array_of_vars['note_text'] ) ? sanitize_textarea_field( wp_unslash( $my_array_of_vars['note_text'] ) ) : '';

		// Validate inputs
		if ( ! $id || ! $target || ! $note_text ) {
			PW_Report_AWR_Helpers::json_error( 'Missing required parameters', 400 );
		}

		// Validate target type
		$allowed_targets = array( 'order', 'product' );
		...
		if ( 'order' === $target ) {
			...
			if ( ! $order ) {
				PW_Report_AWR_Helpers::json_error( 'Invalid order', 404 );
			}
		...
		PW_Report_AWR_Helpers::json_success( array(
			'message' => esc_html__( 'New note has been saved.', __PW_REPORT_WCREPORT_TEXTDOMAIN__ ),
		) );
	}

	/**
	 * AJAX Handler: Change Order Status
	 *
	 * @since 6.1
	 * @return void
	 */
	...
	function pw_rpt_int_change_order_staus() {
		...
		$order_id = isset( $my_array_of_vars['order_id'] ) ? absint( $my_array_of_vars['order_id'] ) : 0;
		$status   = isset( $my_array_of_vars['status'] ) ? sanitize_text_field( $my_array_of_vars['status'] ) : '';
		...
		if ( ! in_array( $status, $valid_statuses, true ) && ! in_array( $status_with_prefix, $valid_statuses, true ) ) {
			PW_Report_AWR_Helpers::json_error( 'Invalid order status', 400 );
		}
		...
		$order->update_status( $status, __( 'Order status changed via reports.', __PW_REPORT_WCREPORT_TEXTDOMAIN__ ) );

		PW_Report_AWR_Helpers::json_success( array(
			'message' => esc_html__( 'Order status updated successfully.', __PW_REPORT_WCREPORT_TEXTDOMAIN__ ),
		) );
	}


	/**
	 * AJAX Handler: Fetch Single Customer
	 *
	 * @since 6.1
	 * @return void
	 */
	...
	function pw_rpt_fetch_single_customer() {
		...
		$page_id          = isset( $my_array_of_vars['page_id'] ) ? absint( $my_array_of_vars['page_id'] ) : 0;
		$customer_id      = isset( $my_array_of_vars['customer_id'] ) ? absint( $my_array_of_vars['customer_id'] ) : 0;
		$customer_email   = isset( $my_array_of_vars['customer_email'] ) ? sanitize_email( $my_array_of_vars['customer_email'] ) : '';
		$customer_segment = isset( $my_array_of_vars['customer_segment'] ) ? sanitize_text_field( $my_array_of_vars['customer_segment'] ) : '';

		global $pw_rpt_main_class;
		include( __PW_REPORT_WCREPORT_ROOT_DIR__ . '/includes/Intelligence/fetch_data_int_reports_customers_single.php' );
	...
	/**
	 * AJAX Handler: Fetch Single Product
	 *
	 * @since 6.1
	 * @return void
	 */
	...
	function pw_rpt_fetch_single_product() {
		...
		$product_id                 = isset( $my_array_of_vars['product_id'] ) ? absint( $my_array_of_vars['product_id'] ) : 0;
		$product_rank_type          = isset( $my_array_of_vars['product_rank_type'] ) ? sanitize_text_field( $my_array_of_vars['product_rank_type'] ) : '';
		$product_rank_val           = isset( $my_array_of_vars['product_rank_val'] ) ? sanitize_text_field( $my_array_of_vars['product_rank_val'] ) : '';
		$product_rank_title         = isset( $my_array_of_vars['product_rank_title'] ) ? sanitize_text_field( $my_array_of_vars['product_rank_title'] ) : '';
		$pw_from_date               = isset( $my_array_of_vars['pw_from_date'] ) ? sanitize_text_field( $my_array_of_vars['pw_from_date'] ) : '';
		$pw_to_date                 = isset( $my_array_of_vars['pw_to_date'] ) ? sanitize_text_field( $my_array_of_vars['pw_to_date'] ) : '';
		$total_products_amnt        = isset( $my_array_of_vars['total_products_amnt'] ) ? floatval( $my_array_of_vars['total_products_amnt'] ) : 0;
		$total_products_refund_amnt = isset( $my_array_of_vars['total_products_refund_amnt'] ) ? floatval( $my_array_of_vars['total_products_refund_amnt'] ) : 0;

		global $pw_rpt_main_class;
		include( __PW_REPORT_WCREPORT_ROOT_DIR__ . '/includes/Intelligence/fetch_data_int_reports_products_single.php' );

		wp_die();
	}

	/**
	 * AJAX Handler: Add/Remove Favorite Menu
	 *
	 * @since 6.1
	 * @return void
	 */
	...
	function pw_rpt_add_fav_menu() {
		...
		$smenu = isset( $my_array_of_vars['smenu'] ) ? sanitize_text_field( $my_array_of_vars['smenu'] ) : '';

		if ( empty( $smenu ) ) {
			PW_Report_AWR_Helpers::json_error( 'Missing menu parameter', 400 );
		}

		$current_user      = wp_get_current_user();
		$user_info         = sanitize_user( $current_user->user_login );
		$current_fav_menus = PW_Report_AWR_Helpers::get_option( 'fav_menus_' . $user_info, array() );

		if ( ! is_array( $current_fav_menus ) ) {
			$current_fav_menus = array();
		}

		if ( in_array( $smenu, $current_fav_menus, true ) ) {
			$current_fav_menus = array_diff( $current_fav_menus, array( $smenu ) );
		} else {
			$current_fav_menus[] = $smenu;
		}

		PW_Report_AWR_Helpers::update_option( 'fav_menus_' . $user_info, $current_fav_menus );

		PW_Report_AWR_Helpers::json_success( array( 'smenu' => $smenu ) );
	}
...
//print_r($product_array);
	...
	/**
	 * AJAX Handler: Test Email Functionality
	 *
	 * @since 6.1
	 * @return void
	 */
	...
	function pw_rpt_test_email() {
		...
		PW_Report_AWR_Helpers::json_success( array( 'html' => $html ) );
	}
