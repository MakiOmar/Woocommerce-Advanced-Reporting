...
if ($file_used == "sql_table") {
    ...
    if ($pw_detail_view == "yes") {
        ...
        $columns = array(
            ...
            array('lable' => esc_html__('Customer Notes', __PW_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'show'),
        );
    ...
    foreach ((array)$this->results as $items) {
        ...
        if ($pw_detail_view == "yes") {
            if ($new_order) {
                ...
                $datatable_value .= ("<td style='" . $display_class . "'>");
				$bags = array("Small Bag/5", "Small Bag/3", "Small Bag/2", "not bag", "ِِSmall Bag/5");
				if (!in_array($sku, $bags)) {
					$datatable_value .= $sku;
				} else {
					$datatable_value .= "Small Bag";
				}
                $datatable_value .= ("</td>");
                ...
                if ($has_serialized_data && !$is_individually_sold) {
                    for ($x = 0; $x < count($has_serialized_data); $x++) {
                        ...
                        if ($addon_id[0] == "product") {
                            ...
                            $product_name = $product->get_name();
                            $product_category = $this->pw_get_cn_product_id($product->get_id(), "product_cat");
                            ...
                            $datatable_value .= ("<td style='" . $display_class . "'>");
							$bags = array("Small Bag/5", "Small Bag/3", "Small Bag/2", "not bag", "ِSmall Bag/5");
							if (!in_array($product_sku, $bags)) {
                            ...
                            $datatable_value .= ("<td style='" . $display_class . "'>");
                            $datatable_value .= $product_category;
                            $datatable_value .= ("</td>");
                            ...
                            if ( $product->is_type( 'variable' ) ) {
                                $datatable_value .= urldecode($variationName);
                            } else {
                                $datatable_value .= urldecode($product->get_slug());
                            }
                            $datatable_value .= ("</td>");
                            ...
                            $datatable_value .= ("<td style='" . $display_class . "'>");
                            if ( $product->is_type( 'variable' ) ) {
                                $datatable_value .= $this->sizes[urldecode($variationName)];
                            } else {
                                $datatable_value .= $this->sizes[urldecode($product->get_slug())];
                            }
