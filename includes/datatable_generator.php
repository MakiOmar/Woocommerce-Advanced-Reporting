<?php
	if(!class_exists('pw_rpt_datatable_generate'))
	{
		...
		/**
		 * Get common report filters from request
		 *
		 * @since 6.1
		 * @return array Common filter parameters
		 */
...
//							array('lable'=>esc_html__('Email',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
		...
		public function table_columns_total($table_name){
			...
			switch($table_name){
				...
				case 'details_no_items':
					...
					case 'details_user_id_with_items':
						$table_column=array(
							array('lable'=> esc_html__('Order Count',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
							array('lable'=> esc_html__('Product Count',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
							array('lable'=> esc_html__('Qty.',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
			...
			public function table_html_pdf($table_name,$search_fields=NULL,$selected_columns=array()){
				...
				if($table_name=='refunddetails')
				{
					$refund_type=$this->refund_status;
					$this->table_cols =$this->table_columns($refund_type);
				}else if( in_array ( $table_name,array('details','details_depot','details_user_id','details_full','details_full_shipping','details_full_shipping_tax','abandoned_cart','abandoned_product','details_product_options','product','product_per_users','category','tags','details_order_country','details_brands','details_tax_field','prod_per_month','variation_per_month','prod_per_country','prod_per_state','country_per_month','payment_per_month','ord_status_per_month','summary_per_month','variation','stock_summary_avg','stock_list','stock_list_sales','variation_stock','tax_reports','customer_role_total_sale','customer_role_top_products','customer_role_bottom_products','details_tickera','stock_zero_level','stock_max_level') ) )
				{
					//$this->table_cols =$this->table_columns($table_name);
				}else{
		...
		function pw_get_prod_stock_($order_item_id, $pw_product_id){
			...
			$pw_table_value = strlen((string)$pw_table_value) > 0 ? $pw_table_value : $this->pw_op_stock($pw_product_id);
			$pw_table_value = strlen((string)$pw_table_value) > 0 ? $pw_table_value : 'Not Set';
				return $pw_table_value;
			}
			...
			function pw_get_prod_custom_fields($order_item_id, $pw_product_id,$field/*,$current_page='',$product_type='-1'*/){
				...
				$pw_table_value = strlen((string)$pw_table_value) > 0 ? $pw_table_value : $this->pw_get_op_custom_fields($pw_product_id,$field);
			...
			function pw_get_prod_custom_fields_2($order_item_id, $pw_product_id,$field/*,$current_page='',$product_type='-1'*/){
				...
				$pw_table_value = strlen((string)$pw_table_value) > 0 ? $pw_table_value : $this->pw_get_op_custom_fields_2($pw_product_id,$field);
...
//                    }
			...
			// Suppress type narrowing warning by using proper type and error suppression for JIT
			$sum = floatval($v1 + $v2);
