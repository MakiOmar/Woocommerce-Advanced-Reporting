...
if ( ! class_exists('pw_report_wcreport_class')) {
    ...
    define('__PW_REPORT_WCREPORT_TEXTDOMAIN__', 'woocommerce-advanced-reporting');

    //COST OF GOOF PRICE
    //define ('__PW_COG__','_PW_COST_GOOD_FIELD');

    include('includes/datatable_generator.php');
    include('includes/class-awr-helpers.php');
    ...
    function __construct()
        {
            ...
            $this->sizes         = array(
				...
				"مجموعة-سدس-تولة" => 2,
				"z-مجموعة-نصف-تولة" => 6,
				"15-تولة-مقسمة-أرباع" => 180,
                "3-تولات-مقسمة-أرباع" => 36,
                "5-تولات-مقسمة-أرباع" => 60,
                "8-جرام" => 8,
				"بدون-أكياس" => 0,
				"كيسين" => 2,
				"3-أكياس" => 3,
...
//			}
        ...
        {
            load_plugin_textdomain('woocommerce-advanced-reporting', false,
                dirname(plugin_basename(__FILE__)) . '/languages/');
        }
...
where pitems.order_id='$order_id' AND pmeta.meta_key='_wc_eco_fields_value' AND pmeta_p.meta_key='_wc_eco_element_id' AND pmeta_p.meta_value='$field'";
         ...
         */
        function pw_report_fix_admin_title_global() {
            global $title;
            
            // If global title is null or empty, set a default to prevent strip_tags error
            if (empty($title)) {
        ...
        //Details with user id
        function wcx_plugin_menu_details_user_id()
        {
            $this->pages_fetch("details_user_id.php");
        }
            ...
            $this->our_menu = array(
                ...
                "all_order_reports" => array(
                    ...
                    "childs" => array(
                        ...
                        "all_orders_user_id"               => array(
                            "label" => esc_html__('تقرير  خدمة العملاء', __PW_REPORT_WCREPORT_TEXTDOMAIN__),
                            "id"    => "all_orders_user_id",
                            "link"  => "admin.php?page=wcx_wcreport_plugin_details_user_id&parent=all_order_reports&smenu=all_orders_user_id",
                            "icon"  => "fa-file-text",
                        ),
                ...
                "customer_reports" => array(
                    ...
                    "childs" => array(
                        ...
                        "customer_guest"              => array(
                            "label" => esc_html__("Guest Customer", __PW_REPORT_WCREPORT_TEXTDOMAIN__),
                            "id"    => "customer_guest",
                            "link"  => "admin.php?page=wcx_wcreport_plugin_customer&parent=customer_reports&smenu=customer_guest",
                            "icon"  => "fa-user",
                        ),
                        ////ADDED IN VER4.0
                        //OTHER SUMMARY
